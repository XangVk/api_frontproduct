...
export default function SellProduct() {
    ...
    async function getproductsell() {
        try {
            ...
            const data = await axios.get("http://164.92.69.177:3001/productsell/getproductsellbysell", { params: key, headers: { Authorization: token } });
            if (data.status == 200) {
...
return;
            ...
            const token = localStorage.getItem("token");
            console.log(productadd)
            const data = await axios.post("http://164.92.69.177:3001/sellproduct/createsellproduct", productadd, { headers: { Authorization: token } });
            console.log(data.data)
            if (data.status = 200) {
                console.log(data.data)
                    ...
                    <div className="w-55 h-100  ">
                        <div className="w-50 d-flex flex-direction-column pl-6 pb-1">
                            ...
                            <div className="position-relative ">
                                ...
                                />
                                <img src="http://164.92.69.177:3001/image/post/icon_search.png" className="h-30-px position-absolute top-0  right-42-l cursor-pointer" />
                            </div>
                        </div>
                                ...
                                productsell.length > 0 && productsell.map((item, index) =>
                                    <div className="w-30 max-height-213-px mx-3 max-width-180-px  position-relative z-index-1 mb-4">
                                        <div className=" d-flex flex-direction-column  box-shadow-default overflow-hidden position-relative z-index-1" onClick={() => addproduct({ item: item })}>
                                            <img src={`http://164.92.69.177:3001/image/post/${item.image != "false" ? item.image : "noimage.jpg"}`} className="w-100 h-110-px min-h-110-px position-relative z-index-1" />
