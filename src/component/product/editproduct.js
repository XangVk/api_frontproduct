...
export default function EditProducts({ datas, editdata }) {
    ...
    async function onedit(e) {
        try {
            ...
            const datas = await axios.put(`http://localhost:3001/product/updateproduct/${values._id}`, formdata);
            if (datas.status == 200) {
        ...
        <>
            ...
            <div className="w-350-px bg-blue pb-7 pt-5 border-radius-3-px">
                ...
                <div className="text-center position-relative w-100-px m-auto">
                    <img src={imageurledit == null ? `http://localhost:3001/image/post/${values.image}` : imageurledit != null ? imageurledit : `http://localhost:3001/image/post/${values.image}`} className="w-100-px h-100-px border-radius-50" />
