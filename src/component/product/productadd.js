...
export default function Productadd() {
    ...
    async function getproductby() {
        try {
            ...
            const data = await axios.get("http://64.92.98.3:3001/product/getproductby", { params: keyword, headers: { Authorization: token } });
            console.log(data)
            if (data.status == 200) {
                console.log(data.data);
                setproduct(data.data)
    ...
    async function onaddproduct() {
        try {
            ...
            const data = await axios.post("http://64.92.98.3:3001/product/createproductimage", formdata, { headers: { Authorization: token } });
    ...
    async function deleteproduct(id) {
        try {
            const token = localStorage.getItem("token");
            const data = await axios.delete(`http://64.92.98.3:3001/product/deleteproduct/${id}`, { headers: { Authorization: token } });
            if (data.status == 200) {
                getproductby();
                // console.log(data.data)
                    ...
                    <div className="w-40   ">
                        ...
                        <div className="w-60 d-flex flex-direction-column m-auto max-width-300-px pt-3 overflow-hidden mt-5 bg-gray-default-1 px-5 border-radius-3-px py-7 mt-3">
                            ...
                            <div className="text-center position-relative">
                                <img for="images" src={`${imageurl == null ? `http://64.92.98.3:3001/imageposts/` : imageurl}`} className="w-180-px h-180-px border-1-solid-gray border-radius-5-px border-1-solid-image" />
                            ...
                            <table className="tables w-100 ">
                                ...
                                <tbody className="">
                                    {product.length > 0 && product.map((item, index) =>
                                        ...
                                        <tr key={index}>
                                            <td>{index + 1}</td>
                                            <td>{<img src={`http://64.92.98.3:3001/image/post/${item.image}`} className="w-40-px h-40-px border-radius-50 border-1-solid-default" />}</td>
