...
export default function Register() {
    ...
    async function onregister() {
        try {
            ...
            const user = await axios.post("http://147.182.204.175:3001/user/createuser", formdata);
            if (user.status === 200) {
    ...
    async function getuser() {
        try {
            ...
            const data = await axios.get("http://147.182.204.175:3001/user/getuser");
            if (data.status == 200) {
    ...
    async function deleteuser(id) {
        try {
            const data = await axios.delete(`http://147.182.204.175:3001/user/deleteuser/${id}`);
            if (data.status == 200) {
                getuser();
            }
        } catch (error) {
                ...
                <div className="w-40  h-95-vh px-3 d-flex ">
                    ...
                    <div className="w-40  px-3 d-flex flex-direction-column ">
                        ...
                        <div className="w-100 d-flex flex-direction-column ">
                            <img src={`${imageurls == null ? `http://147.182.204.175:3001/image/posts/` : imageurls}`} className="w-100 border-1-solid-default h-150-px border-radius-3-px max-width-180-px" />
                        ...
                        <tbody>
                            {
                                user && user.length > 0 && user.map((item, index) =>
                                    <tr key={index}>
                                        <td> <img src={item.image != "false" ? `http://147.182.204.175:3001/image/post/${item.image}` : `http://147.182.204.175:3001/image/post/noimage.jpg`} className="w-30-px h-30-px border-radius-50" /> </td>
                                        <td >{item.username} </td>
