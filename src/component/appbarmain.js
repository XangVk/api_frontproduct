   ...
   const refresh = async ({ data }) => {
      try {
         ...
         if (data != null) {
            ...
            const datalogin = await axios.post("http://164.92.69.177:3001/user/refresh", datas);
         ...
         {logout ? <>
           ...
           <>
               <div className=" h-50-px bg-default py-2 pl-3 d-flex justify-content-between">
                  <div >
                     <img src={`http://164.92.69.177:3001/image/post/logo_2.png`} className="h-50-px cursor-pointer" onClick={() => mainpage({mains : false})} />

                  </div>
                  <div className=" d-flex align-items-center pr-5">
                     <img src="http://164.92.69.177:3001/image/post/user.png" className="w-30-px h-30-px mr-2" />
